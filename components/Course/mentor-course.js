    ...
    return (
        <section>
            <div className="container">
                {course ? (
                    <div className="grid grid-cols-3  mt-10 TeamCurseMaster" key={course.id}>
                        <div className=" relative col-span-2 ">
                            <h6 className="capitalize leading-18 text-[12px] text-[#9099A3]">КУРСТУН ТРЕНЕРИ</h6>
                            <h1 className="title_hero text-white leading-relaxed   font-bold">
                                {course.mentor.full_name}
                            </h1>
                            <p className="capitьalize leading-18 text-[16px] font-[500] text-[white] "  dangerouslySetInnerHTML={{ __html: course.mentor[`short_bio_${locale}`]}}/>
                            <p className="capitalize leading-18 text-[16px] font-[500] text-[white] mt-1"  dangerouslySetInnerHTML={{ __html: course.mentor[`bio_${locale}`]}}/>


                        </div>
                        <div className="DontworkingMaster_block ">
                            <img src={course.mentor.image} alt="Фото тренера" className="mentor-img"/>
                        </div>
                    </div>
                ) : (
